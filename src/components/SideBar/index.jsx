...
import { hideForm, hideProjectView, setSelectedProject, showForm, showProjectView } from "../actions/projectActions";
...
function SideBar() {
  ...
  return (
    <aside className="sidebar lg:left-0 p-2 w-[300px] overflow-y-auto text-center bg-gray-700 rounded-r-xl mt-6">
      ...
      {projects.length > 0 && projects.map((project, i)=> <div className="py-2 px-4 my-1 cursor-pointer text-white bg-gray-600 rounded hover:bg-gray-500" key={i} onClick={() => handleProjectView(project)}>{project.name}</div>)}
    </aside>
  );  
}
